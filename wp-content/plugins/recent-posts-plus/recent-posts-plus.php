...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        'wp_query_options' => '',
        'widget_output_template' => '<li>{THUMBNAIL}<a title="{TITLE_RAW}" href="{PERMALINK}">{TITLE}</a>{EXCERPT}</li>',
//default format
    ...
    function widget( $args, $instance ) {
        ...
        if(stripos(ltrim($widget_output_template), '<li') === 0)
            echo '<ul>'.$output.'</ul> <p><a href="/resources/news-updates/" class="roll">Read More</a></p>';
        else
            echo $output;
